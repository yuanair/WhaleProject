	...
	uint64 WWindow::OnMessage(uint32 msg, uint64 wParam, int64 lParam)
	{
		switch (msg)
		{
			...
			case WM_IME_STARTCOMPOSITION:
			{
				...
				else FDebug::LogError(WProgram::GetAppNameW(), L"ImmGetContext Failed");
			...
			case WM_IME_COMPOSITION:
			{
				...
				if (lParam & GCS_RESULTSTR)
				{
					...
					if (hstr == nullptr) FDebug::LogError(WProgram::GetAppNameW(), L"GlobalAlloc Failed");
					
					lpstr = (LPTSTR) GlobalLock(hstr);
					if (lpstr == nullptr) FDebug::LogError(WProgram::GetAppNameW(), L"GlobalLock Failed");
