...
namespace Whale
{
	...
	WRendererDirectX::WRendererDirectX()
	{
		this->data.New();
	}
	...
	void WRendererDirectX::Create()
	{
		...
		// 创建dxgi工厂
		THROW_IF_FAILED(
			CreateDXGIFactory2(dxgiFactoryFlags, IID_PPV_ARGS(&data->pIDXGIFactory))
		);
		
		// 遍历适配器
		for (UINT adapterIndex = 0;
			 DXGI_ERROR_NOT_FOUND != data->pIDXGIFactory->EnumAdapters1(adapterIndex, &pIAdapter); ++adapterIndex)
		{
		...
		// 创建D3D12.1的设备
		THROW_IF_FAILED(
			D3D12CreateDevice(pIAdapter.Get(), D3D_FEATURE_LEVEL_12_1, IID_PPV_ARGS(&data->pID3D12Device))
		);
		...
		THROW_IF_FAILED(
			data->pID3D12Device->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&data->pID3D12CommandQueue))
		);
		...
		if (FAILED(
			data->pID3D12Device->CheckFeatureSupport(
		...
		THROW_IF_FAILED(data->pID3D12Device->CreateRootSignature(
			0, pISignatureBlob->GetBufferPointer(), pISignatureBlob->GetBufferSize(),
			IID_PPV_ARGS(&data->pID3D12RootSignature)));
		
		// 得到每个描述符元素的大小
		data->nRTVDescriptorSize = data->pID3D12Device->GetDescriptorHandleIncrementSize(
			D3D12_DESCRIPTOR_HEAP_TYPE_RTV
		);
		
		// 创建命令列表分配器
		THROW_IF_FAILED(
			data->pID3D12Device->CreateCommandAllocator(
				D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&data->pID3D12CommandAllocator))
		);
		// 创建图形命令列表
		THROW_IF_FAILED(
			data->pID3D12Device->CreateCommandList(
				0, D3D12_COMMAND_LIST_TYPE_DIRECT, data->pID3D12CommandAllocator.Get(), nullptr,
				IID_PPV_ARGS(&data->pID3D12CommandList))
		);
		// 创建一个同步对象——围栏，用于等待渲染完成，因为现在Draw Call是异步的了
		THROW_IF_FAILED(
			data->pID3D12Device->CreateFence(
				0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&data->pID3D12Fence)));
		data->n64FenceValue = 1;
		// 创建一个Event同步对象，用于等待围栏事件通知
		data->hFenceEvent = CreateEvent(nullptr, FALSE, FALSE, nullptr);
		if (data->hFenceEvent == nullptr)
		{
	...
	void WRendererDirectX::EnableDebugLayer() const
	{
#if defined(DEBUG) || defined(_DEBUG)
		THROW_IF_FAILED(D3D12GetDebugInterface(IID_PPV_ARGS(&data->pID3D12Debug)));
		data->pID3D12Debug->EnableDebugLayer();
#endif
	}
