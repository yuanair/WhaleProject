	...
	namespace WRL
	{
		namespace Details
		{
			...
			public:
				...
				{
					static_assert(
						__is_base_of(IUnknown, InterfaceType),
			...
			template<typename T>
			class ComPtrRef
				: public Details::ComPtrRefBase<T> // T should be the ComPtr<T> or a derived type of it, not just the interface
			{
		...
		namespace Details
		{
			...
			protected:
				template<typename T>
				static HRESULT
				AsIID(_In_ T *implements, REFIID riid, _Outptr_result_nullonfailure_ void **ppvObject) noexcept
				{
					*ppvObject = nullptr;
...
#pragma warning(pop)
			...
			protected:
				HRESULT CanCastTo(REFIID riid, _Outptr_ void **ppv) throw()
				{
					...
					return (
						       InterfaceTraits<I0>::CanCastTo(this, riid, ppv) ||
			...
			protected:
				HRESULT
				CanCastTo(_In_ REFIID /*riid*/, _Outptr_ void ** /*ppv*/, bool * /*pRefDelegated*/ = nullptr) noexcept
				{
		...
		protected:
			HRESULT CustomQueryInterface(REFIID /*riid*/, _Outptr_result_nullonfailure_ void ** /*ppvObject*/, _Out_
			                             bool *handled)
			{
				*handled = false;
				return S_OK;
