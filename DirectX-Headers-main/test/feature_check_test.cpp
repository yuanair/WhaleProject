...
int run_per_adapter(IUnknown *adapter)
{
	...
	// 3: Format Support
	{
		...
		if (SUCCEEDED(
			device->CheckFeatureSupport(
				D3D12_FEATURE_FORMAT_SUPPORT, &Data, sizeof(D3D12_FEATURE_DATA_FORMAT_SUPPORT))))
		{
	...
	// 4: Multisample Quality Support
	{
		...
		if (FAILED(
			features.MultisampleQualityLevels(
				DXGI_FORMAT_R16G16B16A16_FLOAT, 1, D3D12_MULTISAMPLE_QUALITY_LEVELS_FLAG_NONE, NumQualityLevels
			)))
		{
		...
		Data.SampleCount = 1;
		device->CheckFeatureSupport(
			D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &Data, sizeof(D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS));
	...
	// 6: GPU Virtual Address Support
	{
		...
		device->CheckFeatureSupport(
			D3D12_FEATURE_GPU_VIRTUAL_ADDRESS_SUPPORT, &Data, sizeof(D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT));
	...
	// 7: Shader Model
	{
		...
		for (UINT i = 0; i < NumKnownShaderModels; i++)
		{
			...
			if (SUCCEEDED(
				device->CheckFeatureSupport(
					D3D12_FEATURE_SHADER_MODEL, &Data, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL))))
			{
	...
	for (UINT NodeId = 0; NodeId < NodeCount; NodeId++)
	{
		...
		VERIFY_RENAMED_FEATURE_CHECK(ProtectedResourceSessionSupport, Support,
	...
	{
		// Basic check
		BOOL CommandQueuePrioritySupportedPositive = features.CommandQueuePrioritySupported(
			D3D12_COMMAND_LIST_TYPE_DIRECT, D3D12_COMMAND_QUEUE_PRIORITY_NORMAL
		);
		// Check for Global Realtime support
		BOOL CommandQueuePrioritySupportedRealtime = features.CommandQueuePrioritySupported(
			D3D12_COMMAND_LIST_TYPE_COPY, D3D12_COMMAND_QUEUE_PRIORITY_GLOBAL_REALTIME
		);
		// Test on invalid inputs
		BOOL CommandQueuePrioritySupportedInvalid = features.CommandQueuePrioritySupported((D3D12_COMMAND_LIST_TYPE) 7,
		                                                                                   0
		);
		...
		D3D12_FEATURE_DATA_COMMAND_QUEUE_PRIORITY Data;
		{
			...
			{
				PRINT_CHECK_FAILED_MESSAGE(CommandQueuePriority, Data.PriorityForTypeIsSupported,
				                           CommandQueuePrioritySupportedPositive);
			}
		}
		
		{
			...
			{
				PRINT_CHECK_FAILED_MESSAGE(CommandQueuePriority, Data.PriorityForTypeIsSupported,
				                           CommandQueuePrioritySupportedRealtime);
			}
		}
	...
	for (UINT NodeId = 0; NodeId < NodeCount; NodeId++)
	{
		...
		if (FAILED(
			device->CheckFeatureSupport(
				D3D12_FEATURE_PROTECTED_RESOURCE_SESSION_TYPE_COUNT, &CountData,
...
int main()
{
	...
	for (uint32_t i = 0; i < adapter_list->GetAdapterCount(); i++)
	{
		...
		if (FAILED(run_per_adapter(adapter.Read())))
		{
